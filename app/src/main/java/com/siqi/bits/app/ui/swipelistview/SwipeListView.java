...
public class SwipeListView extends ListView {
  ...
  private void init(AttributeSet attrs) {
    ...
    if (attrs != null) {
      ...
      swipeDrawableChecked = styled.getResourceId(R.styleable.SwipeListView_swipeDrawableChecked,
          0);
      swipeDrawableUnchecked = styled.getResourceId(R.styleable
          .SwipeListView_swipeDrawableUnchecked, 0);
    ...
    if (swipeFrontView == 0 || swipeBackView == 0) {
      ...
      if (swipeFrontView == 0 || swipeBackView == 0) {
        throw new RuntimeException(String.format("You forgot the attributes swipeFrontView or " +
                "swipeBackView. You can add this attributes or use '%s' and '%s' identifiers",
    ...
    touchListener = new SwipeListViewTouchListener(getContext(), this, swipeFrontView,
        swipeBackView);
  ...
  protected void onLeftChoiceAction(int position) {
    if (swipeListViewListener != null && position != ListView.INVALID_POSITION) {
      swipeListViewListener.onLeftChoiceAction(position);
    }
  }

  protected void onRightChoiceAction(int position) {
    if (swipeListViewListener != null && position != ListView.INVALID_POSITION) {
      swipeListViewListener.onRightChoiceAction(position);
    }
  }
  ...
  public boolean onInterceptTouchEvent(MotionEvent ev) {
    ...
    Log.d("alpha: swipelistview: onInterceptTouchEvent", MotionEvent.actionToString(action));
    final float x = ev.getX();
    final float y = ev.getY();

    if (isEnabled() && touchListener.isSwipeEnabled()) {
      Log.d("alpha: swipelistview: onInterceptTouchEvent", "Swipe enabled.");
      if (touchState == TOUCH_STATE_SCROLLING_X) {
        boolean onTouch = touchListener.onTouch(this, ev);
        Log.d("alpha: swipelistview: onInterceptTouchEvent", "touchListener.onTouch(this, " +
            "ev)=" + onTouch);
        return onTouch;
      }
      ...
      switch (action) {
        case MotionEvent.ACTION_MOVE:
          checkInMoving(x, y);
          Log.d("alpha: SwipeListView ACTION_MOVE", "Intercepted: " + (touchState ==
        ...
        case MotionEvent.ACTION_DOWN:
          ...
          lastMotionX = x;
          lastMotionY = y;
          Log.d("alpha: SwipeListView TOUCH_STATE_REST", "Intercepted: false");
          return false;
        ...
        case MotionEvent.ACTION_UP:
          touchListener.onTouch(this, ev);
          Log.d("alpha: SwipeListView ACTION_UP", "Intercepted: " + (touchState ==
              TOUCH_STATE_SCROLLING_Y));
          return touchState == TOUCH_STATE_SCROLLING_Y;
        default:
          break;
